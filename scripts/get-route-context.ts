 ...
 *
 *   页面映射模式：
 *     pnpm run run:route-params -- <platform> map=true
 *
 ...
 * map=true : 可选，直接输出该平台完整的 PAGE_ID_MAP，忽略 page 参数
 *
 * 示例：
 ...
 *   pnpm run run:route-params -- frontend map=true
 */
...
const mapOnly = argv.includes('map=true');
const batchMode = argv.length >= 2 && argv[1] === 'batch';

if (!navOnly && !mapOnly && !batchMode && argv.length < 2) {
...
// map=true：直接输出 PAGE_ID_MAP
if (mapOnly) {
  console.log(JSON.stringify(PAGE_ID_MAP, null, 2));
  process.exit(0);
}

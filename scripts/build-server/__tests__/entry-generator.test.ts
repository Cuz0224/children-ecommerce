...
describe('generateEntry', () => {
  ...
  it('should register project routes without exposing the registrar as an RPC action', async () => {
    fs.writeFileSync(path.join(tmpDir, '_common.js'), 'module.exports = {};');
    fs.writeFileSync(
      path.join(tmpDir, 'src.backend.actions.PaymentWebhook.js'),
      'module.exports = { registerProjectRoutes: function() {}, helper: function() {} };'
    );
    ...
    const content = fs.readFileSync(result.outputFile, 'utf-8');
    expect(content).toContain("typeof mod.registerProjectRoutes === 'function'");
    expect(content).toContain('mod.registerProjectRoutes(router)');
    expect(content).toContain("fnName !== 'registerProjectRoutes'");
  });

...
describe('build function', () => {
  ...
  it('should rebuild an empty common bundle in single file mode', async () => {
    const actionFile = 'src/backend/actions/EmptyCommon.ts';
    const outDir = path.join(tmpDir, 'server-action-generated');
    fs.writeFileSync(
      path.join(tmpDir, actionFile),
      'export function emptyCommon() { return 1; }\n'
    );
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, '_common.js'), '');

    const result = await build({
      projectId: 'empty-common',
      file: actionFile,
    });

    expect(result.success).toBe(true);
    expect(result.commonRebuilt).toBe(true);
    expect(fs.statSync(path.join(outDir, '_common.js')).size).toBeGreaterThan(0);
  }, 30000);

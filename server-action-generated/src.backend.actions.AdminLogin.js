...
var adminLogin = (0, import_action_utils.withResult)(
  async (input) => {
    ...
    if (!trimmedUsername || !input.password) {
      throw new import_action_utils.UnauthorizedError("Username and password are required");
    }
    ...
    if (!user) {
      throw new import_action_utils.UnauthorizedError("Invalid administrator credentials");
    }
    if (user.role !== "ADMIN") {
      throw new import_action_utils.UnauthorizedError("Access restricted to administrator accounts only");
    }

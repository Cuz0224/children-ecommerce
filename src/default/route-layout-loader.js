...
/** App 首页识别：home > dashboard > 其它业务页；跳过登录/注册/layout-test */
const APP_HOME_SKIP_SLUGS = new Set([
  'applogin',
  'appregister',
  'login',
  'register',
...
function scoreAppHomeRoute(routePath) {
  const slug = String(routePath || '')
    .replace(/^\//, '')
    .toLowerCase()
  if (!slug || APP_HOME_SKIP_SLUGS.has(slug)) return -1
  if (slug.endsWith('login') || slug.endsWith('register')) return -1
  if (slug.includes('home') || slug === 'apphome') return 300
  if (slug.includes('dashboard')) return 200
  if (slug.includes('index') || slug.includes('main')) return 150
  return 100
}
...
function scanAppRoutes(projectRoot) {
  ...
  routeCache.appPaths = []
  routeCache.routeMap = {}
  routeCache.appHomePath = '/apphome'
  routeCache.appHomeScore = 0
        ...
        if (entry.isDirectory()) {
          ...
          if (relativePath.includes('(backend)')) {
            ...
            if (!routeCache.appPaths.includes(normalizedRoute)) {
              ...
              // 识别 App 首页：home > dashboard > 首个业务页（勿只认 *home*，否则 /appdashboard 会落到错误 /apphome）
              const homeScore = scoreAppHomeRoute(normalizedRoute)
              if (homeScore > (routeCache.appHomeScore || 0)) {
                routeCache.appHomePath = normalizedRoute
                routeCache.appHomeScore = homeScore
              }
            }
          }

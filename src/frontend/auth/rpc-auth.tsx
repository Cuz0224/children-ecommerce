...
export function AuthExpiredDialog() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={close}>
      ...
      <DialogContent className="fixed bottom-4 right-4 left-auto top-auto translate-x-0 translate-y-0 bg-popover text-popover-foreground border-border sm:max-w-md shadow-lg rounded-2xl">
        <DialogHeader className="space-y-2">
          <DialogTitle className="text-foreground text-lg font-bold">
            Authentication Required
          </DialogTitle>
          <DialogDescription className="text-muted-foreground text-sm">
            Your session has expired or requires sign-in. You can sign in with your account to continue exploring toys.
          </DialogDescription>
        </DialogHeader>

...
import { paymentAmountToMinorUnits, paymentCurrencyScale } from './amount';
...
export async function createStripeCheckoutSession(params: CreatePaymentParams): Promise<PaymentResult> {
    ...
    const outTradeNo = `${statsProjectId}:STRIPE_${Date.now()}_${Math.floor(Math.random() * 1000)}`;

    const lowerCurrency = String(currency).trim().toLowerCase();
    const unitAmount = paymentAmountToMinorUnits(
        amount,
        paymentCurrencyScale(lowerCurrency),
        'Stripe',
    );

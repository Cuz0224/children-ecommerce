...
import { normalizePaymentAmount, paymentCurrencyScale } from './amount';
...
export async function createClinkCheckoutSession(params: CreatePaymentParams): Promise<PaymentResult> {
    ...
    const normalizedAmount = normalizePaymentAmount(
        amount,
        paymentCurrencyScale(normalizedCurrency),
        'Clink',
    );
    ...
    const outTradeNo = `${statsProjectId}:CLINK_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    const finalSuccessUrl = stripClinkHostedSessionPlaceholder(successUrl);
    const body = {
        customerEmail: normalizedCustomerEmail,
        uiMode: 'hostedPage',
        originalAmount: normalizedAmount,
        originalCurrency: normalizedCurrency.toUpperCase(),

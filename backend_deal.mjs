...
async function main() {
  ...
  // Local development uses the repository SQLite database. This keeps init
  // deterministic and avoids requiring access to the retired remote MySQL DB.
  if (version === "1.0.0" || version === "3.0.0") {
    const localDbUrl = "file:./dev.db";
    const sqliteSchemaPath = resolve(PROJECT_ROOT, "prisma/sqlite-init.sql");
    const localDbPath = resolve(PROJECT_ROOT, "prisma/dev.db");
    const isNewDatabase = !existsSync(localDbPath);
    if (isNewDatabase && existsSync(sqliteSchemaPath)) {
      run(`sqlite3 ${localDbPath} < ${sqliteSchemaPath}`);
    }
    process.env.DATABASE_URL = localDbUrl;
    console.log("Using local SQLite database: prisma/dev.db");
    run("npx prisma generate");
    // database.sql is a MySQL snapshot and must not be executed against SQLite.
    if (isNewDatabase) {
      run("npx prisma db seed");
    } else {
      console.log("Existing local database found; preserving its data.");
    }
    console.log("Done");
    return;
    /*
